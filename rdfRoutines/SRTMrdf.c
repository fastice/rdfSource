...
*/
   ...
   if(units == NULL) sprintf(unitString,"(-)");
   else sprintf(unitString,"(%s)",units);

   if(dimensions == NULL) sprintf(dimensionString," ");
   else sprintf(dimensionString,"[%s]",dimensions);

   if(value == NULL) value = " ";
   if(op != '=' || op != '<' || op != '>') op = '=';
   if(comment == NULL) { com = " "; comment = " "; }
   else com="! ";
    ...
    RDF *rdfElement(RDF *rdfParams, char *keyword)
{
    ...
    for(i=0; i <= strlen(keyword); i++) lcKeyword[i] = (char)tolower((int)keyword[i]);    

    for(tmp=rdfParams; tmp != NULL; tmp = tmp->next) {
           if(strcmp(lcKeyword,tmp->keyword) == 0) return tmp;
    }
    ...
    char *rdfValue(RDF *rdfParams, char *keyword)
{
    ...
    for(tmp=rdfParams; tmp != NULL; tmp = tmp->next) {
           if(strcmp(lcKeyword,tmp->keyword) == 0) return tmp->value;
    }
    return(NULL); /* No match so return NULL */
}
   ...
   static char *getRDFLine(char *bufferSpace,FILE *fp) 
{
    ...
    while(TRUE) {
       ...
       if(buffer==NULL) { /* EOF so return */
           return result; /* EOF so return */
       }
/*
  First line copy tmp to bufferSpace, ow cat strings
*/
       if(result==NULL) result=strcpy(bufferSpace,buffer); 
       else result=strcat(result,buffer);
/* 
   Strip white space
*/
...
*/           
           ...
           } else error("rdfParse: NULL keyword");
